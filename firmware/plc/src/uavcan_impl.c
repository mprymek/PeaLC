...
int uavcan2_init()
{
    ...
    if (xTaskCreate(uavcan_task, "uavcan", STACK_SIZE_UAVCAN, NULL, TASK_PRIORITY_UAVCAN, &uavcan_task_h) != pdPASS)
    {
...
void uavcan_task(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        for (uint8_t i = 0; i < uavcan_dis_blocks_len; i++)
        {
            ...
            log_debug("<- DI%d-%d@%d = ?", block->index, block->index + block->len - 1, block->node_id);
            if (automation_send_get_dis(block->node_id, block->index, block->len) < 0)
            {
        ...
        for (uint8_t i = 0; i < uavcan_ais_blocks_len; i++)
        {
            ...
            log_debug("<- AI%d-%d@%d = ?", block->index, block->index + block->len - 1, block->node_id);
            if (automation_send_get_ais(block->node_id, block->index, block->len) < 0)
            {
        ...
        for (uint8_t i = 0; i < uavcan_dos_blocks_len; i++)
        {
            ...
            PRINTF("<- DO%d-%d@%d =", block->index, block->index + block->len - 1, block->node_id);
            ...
            PRINTF("\n");
#endif
            if (automation_send_set_dos(block->node_id, block->index, block->digital_vals, block->len, CANARD_TRANSFER_PRIORITY_HIGH) < 0)
            {
        ...
        for (uint8_t i = 0; i < uavcan_aos_blocks_len; i++)
        {
...
#if LOGLEVEL >= LOGLEVEL_DEBUG
            PRINTF("<- AO%d-%d@%d =", block->index, block->index + block->len - 1, block->node_id);
            for (int i = 0; i < block->len; i++)
            {
            ...
            PRINTF("\n");
#endif
            if (automation_send_set_aos(block->node_id, block->index, block->analog_vals, block->len, CANARD_TRANSFER_PRIORITY_HIGH) < 0)
            {
...
{
    uint32_t id = frame->id & (~(CANARD_CAN_FRAME_EFF | CANARD_CAN_FRAME_ERR | CANARD_CAN_FRAME_RTR));
                                        ...
                                        uint8_t source_node_id)
{
    ...
    if (uavcan_automation_should_accept_transfer(ins,
...
void uavcan_user_on_transfer_received(CanardInstance *ins, CanardRxTransfer *transfer)
{
...
{
    log_debug("Node %d uptime = %d", source_node_id, node_status->uptime_sec);
}
...
void automation_on_get_dis_response(uint8_t source_node_id, uint8_t index, bool *values, uint8_t len)
{
    ...
    for (uint8_t i = 0; i < uavcan_dis_blocks_len; i++)
    {
        ...
        if (!(block->node_id == source_node_id && block->index == index && block->len == len))
        {
            continue;
        }
...
uint8_t automation_set_dos(uint8_t source_node_id, uint8_t index, const bool *values, uint8_t len)
{
