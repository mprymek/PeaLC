...
#error uavcan_get_unique_id(...) not implemented for this STM32 family
    ...
    for (int i = 12; i < UAVCAN_PROTOCOL_HARDWAREVERSION_UNIQUE_ID_LENGTH; i++)
    {
        out_uid[i] = 0;
    }
}
...
int uavcan_can_tx(const CanardCANFrame *frame)
{
    ...
    can1message.RTR = (frame->id & CANARD_CAN_FRAME_RTR) ? CAN_RTR_REMOTE : CAN_RTR_DATA;
    can1message.DLC = frame->data_len;

    if (frame->id & CANARD_CAN_FRAME_EFF)
    {
        can1message.IDE = CAN_ID_EXT;
        can1message.ExtId = frame->id & (~(CANARD_CAN_FRAME_EFF | CANARD_CAN_FRAME_ERR | CANARD_CAN_FRAME_RTR));
    }
    else
    {
        can1message.IDE = CAN_ID_STD;
        can1message.StdId = frame->id & (~(CANARD_CAN_FRAME_EFF | CANARD_CAN_FRAME_ERR | CANARD_CAN_FRAME_RTR));
    }
    ...
    if ((res = HAL_CAN_AddTxMessage(&hcan, &can1message, (uint8_t *)frame->data, &mailBox)) != HAL_OK)
    {
